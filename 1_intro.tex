...
%QR iteration is regarded as a powerful and effective approach for finding all the singular values.
...
In this paper, we present a new SVD approach called ``Bisection and Twisted'' (BT) algorithm. We use bisection algorithm \cite{95ETNAbisecion} to calculate the singular values, and the twisted algorithm \cite{09NLAAtwisted} to calculate the singular vectors. %replace the inverse iteration in BI, because the twisted algorithm is able to calculate accurate and orthogonal singular vectors. 
